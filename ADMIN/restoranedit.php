...
<?php
	...
	if(isset($_POST['simpan']))
	{
		...
		mysqli_query($connection, "UPDATE restoran SET  restorannama= '$restorannama', restoranalamat='$restoranalamat',restoranketerangan='$restoranketerangan',kategoriharga='$kategoriharga', areaID = '$kodearea' WHERE restoranID = '$restorankode'");
		header("location: restoran.php");
	}

	$dataarea = mysqli_query($connection, "select * from area;");
	$temp = $_GET['ubah'];
	$editrestoran = mysqli_query($connection, "SELECT * FROM restoran WHERE restoranID = '$temp'");
	$rowedit= mysqli_fetch_array($editrestoran);
	$editarea = mysqli_query($connection, "SELECT * FROM restoran, area WHERE restoranID = '$temp' AND restoran.areaID = area.areaID");
...
<!-- flag input -->
  ...
  <div class="form-group row">
    ...
    <div class="col-sm-10">
      <input type="Text" class="form-control" id="koderestoran" name="inputkode" value="<?php echo $rowedit["restoranID"] ?>"   readonly >
    </div>
  </div>
  <div class="form-group row">
    <label for="namarestoran" class="col-sm-2 col-form-label">Nama</label>
    <div class="col-sm-10">
      <input type="Text" class="form-control" id="namarestoran" name="inputnama" value="<?php echo $rowedit["restorannama"] ?>" maxlength="60" required="">
    </div>
  </div>
  <div class="form-group row">
    <label for="alamatrestoran" class="col-sm-2 col-form-label">Alamat</label>
    <div class="col-sm-10">
      <input type="Text" class="form-control" id="alamatrestoran" name="inputalamat" value="<?php echo $rowedit["restoranalamat"] ?>" maxlength="60">
    </div>
  </div>
  <div class="form-group row">
    <label for="keteranganrestoran" class="col-sm-2 col-form-label">Keterangan</label>
    <div class="col-sm-10">
      <input type="Text" class="form-control" id="keteranganrestoran" name="inputketerangan" value="<?php echo $rowedit["restoranketerangan"] ?>" maxlength="255">
    </div>
  </div>
  <div class="form-group row">
